"""  
摘要：  
该代码使用 Manim 库创建一个动画场景，展示一个简单的神经网络结构及其相关的数学公式。主要内容包括：  

1. 线性模型公式：展示线性模型的输出公式 z = w^T x + b，以及相应的损失函数和梯度更新公式。  
2. 神经元结构：通过图形化的方式展示输入层、接收层和输出层的神经元，使用箭头表示连接关系。  
3. 动态输入层：展示一个动态的输入层，其中包含省略号表示输入特征的数量不固定。  
4. 权重和输入矩阵：展示权重矩阵和输入矩阵的数学表示。  
