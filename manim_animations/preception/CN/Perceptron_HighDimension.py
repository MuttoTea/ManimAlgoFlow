"""  
摘要：  
该代码使用 Manim 库创建两个动画场景，展示机器学习中的线性模型和相关概念，包括：  

1. OutputFunctionTransform：展示线性模型的输出函数 z = w^T x + b 的构建过程。通过逐步替换特征和权重，展示如何将特征向量和权重向量结合到输出公式中，并最终展示矩阵形式的表达式。  

2. HyperplaneTransform：展示超平面及其损失函数的不同表示形式，包括二维和四维超平面，以及向量化形式的损失函数。最后引入感知机常用的损失函数，并展示梯度下降更新公式。  
...
class OutputFunctionTransfrom(Scene):
    ...
    def construct(self):
        ...
        # 3) 创建与特征对应的 x_i
        feature1 = (
            MathTex("x_{1}").set_color(RED).next_to(textSepalLen, RIGHT, buff=0.3)
        )
        feature2 = (
            MathTex("x_{2}").set_color(RED).next_to(textSepalWid, RIGHT, buff=0.3)
        )
        feature3 = (
            MathTex("x_{3}").set_color(RED).next_to(textPetalLen, RIGHT, buff=0.3)
        )
        feature4 = (
            MathTex("x_{4}").set_color(RED).next_to(textPetalWid, RIGHT, buff=0.3)
        )
        ...
        groupWxPositions = [
            (
                VGroup(weight1, feature1),
                formula_z[2].get_center(),
            ),  # w1x1 在公式索引2处
            (
                VGroup(weight2, feature2),
                formula_z[4].get_center(),
            ),  # w2x2 在公式索引4处
        ...
        # 9) 创建等号和向量
        text_WT = (
            MathTex(r"\mathbf{W}^{T}", color=BLUE)
            .next_to(text_X, RIGHT, buff=1)
            .scale(0.8)
        )
        textX = (
            MathTex(r"\mathbf{X}", color=GREEN)
            .next_to(text_WT, RIGHT, buff=0.075)
            .scale(0.8)
        )
        ...
        X_matrix = (
            ...
            .next_to(W_matrix, RIGHT, buff=0.2)
            .scale(0.8)
        )
        ...
        # 避免 w1,x1... 残留
        formulaMatrix = (
            MathTex(r"z = \mathbf{w}^T \mathbf{x} + b")
            .move_to(formula_z.get_center())
            .scale(0.8)
        )
        self.play(
...
class HyperplaneTransfrom(Scene):
    def construct(self):
        ...
        self.play(Write(plane_two_var_equation), Write(loss_two_var_equation))
        self.wait(1)

        # 3. 四维超平面及损失函数
        plane_four_var_equation = MathTex(
            r"w_{1} x_{1} + w_{2} x_{2} + w_{3} x_{3} + w_{4} x_{4} + b = 0"
        )
        loss_four_var_equation = MathTex(
