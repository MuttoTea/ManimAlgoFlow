"""  
摘要：  
该代码使用 Manim 库创建一个动画场景，展示感知器模型中的输入计算过程。具体步骤包括：  
1. 显示花萼长度和宽度的文本标签及其对应的变量（x1 和 x2）。  
2. 显示权重变量（w1 和 w2）并将其与输入变量相乘。  
3. 通过加法和等号展示感知器的输入计算公式，最终形成输出 Z 和偏置 b 的表达式。  
4. 动画效果逐步展示每个步骤，帮助观众理解感知器的输入计算过程。  
"""  
