"""  
摘要：  
该代码使用 Manim 库创建一个动画场景，展示两个数学公式之间的比较过程，具体包括：  
1. 显示线性方程的标准形式和感知器模型的输出公式。  
2. 通过变换展示如何将感知器模型的输出公式修改为标准形式。  
3. 高亮显示公式中的对应部分，以帮助观众理解变量之间的关系。  
4. 最后将修改后的公式移动到屏幕上方，结束场景。  
...
class Comparison(Scene):
    def construct(self):
        ...
        # 添加描述文本
        description = Text("寻找合适的w1、w2、b", font_size=26).next_to(
            equation2_modified, DOWN, buff=0.5
        )
        ...
        # 获取公式部分
        eq_parts1 = {
            name: equation1.get_part_by_tex(name) for name in ["A", "x", "B", "y", "C"]
        }
        eq_parts2 = {
            name: equation2_modified.get_part_by_tex(name)
