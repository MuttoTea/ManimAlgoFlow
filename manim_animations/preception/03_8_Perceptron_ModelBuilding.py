...
class AbstractScene(Scene):
    def construct(self):
        ...
        Group1 = VGroup(z_equation, plane_matrix_equation, loss_function_new, gradient_update_w, gradient_update_b)
        # 设置颜色
        input_color = "#6ab7ff"
        receive_color = "#ff9e9e"

        # 创建神经元
        input_layer = VGroup(*[Circle(radius=0.2, fill_opacity=1, color=input_color) for _ in range(4)])
        input_layer.arrange(DOWN, buff=0.7)

        receive_layer = Circle(radius=0.5, fill_opacity=1, color=receive_color).move_to(ORIGIN + UP)
        ...
        # 添加输入和输出标签
        input_labels = VGroup(
            *[MathTex(f"x_{i + 1}").scale(0.8).move_to(input_neuron.get_center()) for i, input_neuron in enumerate(input_layer)])
        ...
        # 动画替换  
        self.play(  
            ReplacementTransform(Group(connections, input_labels), Group(connections_n, labels))  
        )  
        self.wait(2)

        W_matrix = MathTex(r"\begin{bmatrix} w_{1} & w_{1} & \dots  & w_{1} \end{bmatrix}^{T} ").scale(0.8).next_to(receive_layer, DOWN, buff=1.5)
        X_matrix = MathTex(r"\begin{bmatrix} x_{1} & x_{2} & \dots & x_{n} \end{bmatrix}^{T} ").scale(0.8).next_to(W_matrix, DOWN, buff=1)
