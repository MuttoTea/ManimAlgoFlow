...
class HandwritingVisualization(Scene):
    def construct(self):
        ...
        print(f'灰度值: {image_1}')
        # 创建像素组  
        pixel_group_1 = create_pixel_group(image_1)  
        pixel_group_0 = create_pixel_group(image_0)
        ...
        title = Text("如何用感知机识别手写数字").to_edge(UP, buff=0.5).scale(0.6)
        ...
        self.play(FadeIn(width_brace), FadeIn(width_label))  
        self.wait(1)  
        ...
        self.play(FadeOut(width_group), FadeOut(height_group))
        self.wait(1)
        ...
        selected_pixel_gray_value_initial = image_1[select_row, select_col] / 255  # 获取初始灰度值
        ...
        self.play(  
            selected_pixel.animate.set_fill(WHITE, opacity=selected_pixel_gray_value_initial).shift(LEFT*4 + UP*1).scale(1/2.5),  # 仅更改填充颜色
            FadeOut(selected_pixel_text),
            run_time=1  
        )  
