...
class XorCoordinateSystem(Scene):
    def construct(self):
        ...
        # 创建对应颜色的点  
        dots_y1 = [Dot(color=RED, radius=0.15).move_to(cell.get_center()).scale(0.7) for cell in y_1_cells]  
        dots_y0 = [Dot(color=YELLOW, radius=0.15).move_to(cell.get_center()).scale(0.7) for cell in y_0_cells]  
        ...
        # 创建并保存对应颜色的点为独立变量  
        dot_y0_row2 = Dot(color=YELLOW, radius=0.15).move_to(y_0_cells[0].get_center()).scale(0.7)  
        dot_y0_row3 = Dot(color=YELLOW, radius=0.15).move_to(y_0_cells[1].get_center()).scale(0.7)  
        dot_y1_row4 = Dot(color=RED, radius=0.15).move_to(y_1_cells[0].get_center()).scale(0.7)  
        dot_y1_row5 = Dot(color=RED, radius=0.15).move_to(y_1_cells[1].get_center()).scale(0.7)
        ...
        graph1 = axes.plot(lambda x: self.func1(x), x_range=[-0.5, 1.5, 0.01], color=BLUE)
        graph2 = axes.plot(lambda x: self.func2(x), color=BLUE)
        graph3 = axes.plot(lambda x: self.func3(x), color=BLUE)
        graph4 = axes.plot(lambda x: self.func4(x), x_range=[-0.5, 1.5, 0.01], color=BLUE)
        ...
        Group_2 = VGroup(axes, graph1, graph4, dot_y0_row2, dot_y0_row3, dot_y1_row4, dot_y1_row5, axes_labels)
        ...
        ## 创建神经元1
        # 输入层
        input_layer_1 = VGroup(*[Circle(radius=0.2, fill_opacity=1, color=input_color) for _ in range(2)])
        input_layer_1.arrange(DOWN, buff=1.5)

        # 接受层
        receive_layer_1 = Circle(radius=0.5, fill_opacity=1, color=receive_color).move_to(ORIGIN + UP)
        ...
        input_labels_1 = VGroup(
            *[MathTex(f"x_{i + 1}").move_to(input_neuron.get_center()) for i, input_neuron in enumerate(input_layer_1)])
        ...
        p1 = VGroup(input_labels_1, connections_1, receive_layer_1, arrow_1, receive_layer_1, output_label_y_1)

        ## 创建神经元2
        # 输入层
        input_layer_2 = VGroup(*[Circle(radius=0.2, fill_opacity=1, color=input_color) for _ in range(2)])
        input_layer_2.arrange(DOWN, buff=1.5)

        # 接受层
        receive_layer_2 = Circle(radius=0.5, fill_opacity=1, color=receive_color).move_to(ORIGIN + UP)
        ...
        input_labels_2 = VGroup(
            *[MathTex(f"x_{i + 1}").move_to(input_neuron.get_center()) for i, input_neuron in enumerate(input_layer_2)])
        theta = np.deg2rad(0)
        direction = np.array([np.cos(theta), np.sin(theta), 0])
        ...
        p2 = VGroup(input_labels_2, connections_2, receive_layer_2, arrow_2, receive_layer_2, output_label_y_2)

        # 创建神经元3
        # 输入层
        input_layer_3 = VGroup(*[Circle(radius=0.2, fill_opacity=1, color=input_color) for _ in range(2)])
        input_layer_3.arrange(DOWN, buff=1.5)

        # 接受层
        receive_layer_3 = Circle(radius=0.5, fill_opacity=1, color=receive_color).move_to(ORIGIN + UP)
        ...
        input_labels_3 = VGroup(
            *[MathTex(f"x_{i + 1}").move_to(input_neuron.get_center()) for i, input_neuron in enumerate(input_layer_3)])
        theta = np.deg2rad(0)
        direction = np.array([np.cos(theta), np.sin(theta), 0])
        ...
        x1 = MathTex("x_{1}").move_to(receive_layer_1.get_center()+LEFT*2).scale(0.6)
        x2 = MathTex("x_{2}").move_to(receive_layer_2.get_center()+LEFT*2).scale(0.6)
        ...
        self.play(
            Transform(Group(arrow_1, arrow_2, output_label_y_1, output_label_y_2), Group(arrow_new_1, arrow_new_2)),
            FadeIn(p3)
            )
        self.wait(1)

        self.play(
            Transform(Group(connections_1, connections_2), Group(input_arrow_1_1, input_arrow_1_2, input_arrow_2_1, input_arrow_2_2)),
